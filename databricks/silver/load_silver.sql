...
CREATE OR REPLACE TABLE datawarehouse.silver.crm_cust_info AS
SELECT
  cst_id, cst_key,
...
CREATE OR REPLACE TABLE datawarehouse.silver.crm_prd_info AS
SELECT
  prd_id,
...
CREATE OR REPLACE TABLE datawarehouse.silver.crm_sales_details AS
SELECT
...
CREATE OR REPLACE TABLE datawarehouse.silver.erp_cust_az12 AS
SELECT
  ...
  CASE WHEN UPPER(TRIM(gen)) IN ('F','FEMALE') THEN 'Female'
       ...
       ELSE 'n/a' END AS gen
FROM datawarehouse.bronze.erp_cust_az_12;

CREATE OR REPLACE TABLE datawarehouse.silver.erp_loc_a101 AS
SELECT
  REPLACE(cid, '-', '') AS cid,
...
FROM datawarehouse.bronze.erp_loc_a_101;

CREATE OR REPLACE TABLE datawarehouse.silver.erp_px_cat_g1v2 AS
SELECT id, cat, subcat, maintenance
FROM datawarehouse.bronze.erp_px_cat_g_1_v_2;