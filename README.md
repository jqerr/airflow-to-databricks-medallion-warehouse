...
This project was originally built on **Airflow, running in Docker**, orchestrating a SQL Server data warehouse. I later advanced it by migrating the entire stack to Databricks.

**One-liner:** Migrated an existing SQL Server + Airflow (Docker) data warehouse to a cloud-native Databricks + AWS stack — re-platforming stored-procedure ETL logic into Spark SQL, wiring up Unity Catalog's connection to S3 via IAM, and replacing the Dockerized Airflow orchestration with a native Databricks Job.
...
- **Apache Airflow / Docker** — original orchestration engine, containerized via Docker; later replaced by Databricks Jobs
